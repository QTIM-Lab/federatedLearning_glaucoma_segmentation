...
def main():
    ...
    for fl_round in range(fl_rounds):
        ...
        for site_index, (model, train_loader, val_loader, optimizer, scheduler) in enumerate(zip(models, train_dataloaders, val_dataloaders, optimizers, schedulers)):
        ...
        if updated:
        # Save the best global model when there's an update in the best metric
            save_model(models[0], model_directory, fl_round, global_best_val_metric)  # Assuming you want to save the first model as the global model
