...
def main():
    ...
    start_epoch = latest_epoch + 1 if latest_checkpoint else 1

    if latest_checkpoint:
        print(f"Resuming training from checkpoint: {latest_checkpoint}, epoch: {latest_epoch}")
        # Ensure to pass only the checkpoint path to torch.load
        checkpoint = torch.load(latest_checkpoint, map_location=device)
        for model in models:
            model.load_state_dict(checkpoint)
    else:
        print("No checkpoint found, starting training from scratch.")
        start_epoch = 1  # Start from the beginning if no checkpoint is found
