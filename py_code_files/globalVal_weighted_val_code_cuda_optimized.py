...
def total_validation_samples(val_datasets):
    return sum(len(dataset) for dataset in val_datasets)
...
def main():
    ...
    new_base_directory = "/sddata/data/retina_datasets_preprocessed/fl_glaucoma_checkpoints"
    ...
    # Assuming find_latest_checkpoint now returns both checkpoint and epoch
    latest_checkpoint, latest_epoch = find_latest_checkpoint(model_directory)
    # If no checkpoint is found, start from the beginning
    start_epoch = latest_epoch + 1 if latest_checkpoint else 0
    ...
    total_val_samples = total_validation_samples(val_datasets)
    ...
    for epoch in range(start_epoch, args.num_epochs):
        ...
        weights = []
        model = None
        for i, (train_loader, val_loader) in enumerate(zip(train_dataloaders, val_dataloaders)):
            ...
            # Load the checkpoint into the model only if it corresponds to this loop iteration
            if latest_checkpoint and i == 0:  # Assuming you're saving/loading checkpoints per model
                checkpoint = torch.load(latest_checkpoint, map_location=device)
                model.load_state_dict(checkpoint)
                print(f"Resuming training from checkpoint for model {i+1}: {latest_checkpoint}")
        ...
        # Initialize a variable to accumulate weighted validation loss
        weighted_val_loss_sum = 0.0
        
        for i, val_loader in enumerate(val_dataloaders):
            val_loss, mean_iou = validate_model(model, val_loader, preprocessor, criterion, device, metric, palette, inference_directory, epoch, args.num_val_outputs_to_save, id2label)
            site_weight = len(val_loader.dataset) / total_val_samples
            print(f"Validation Loss: {val_loss}, Mean IoU: {mean_iou}")
            weighted_val_loss = val_loss * site_weight
            weighted_val_loss_sum += weighted_val_loss
            total_val_loss += val_loss
            total_mean_iou += mean_iou
            print(f"Site {i+1} - Validation Loss: {val_loss}, Weighted Validation Loss: {weighted_val_loss}, Mean IoU: {mean_iou}")
            dataset_name = dataset_names[i]
            individual_val_losses[dataset_name].append(val_loss)
            epoch_log_files[dataset_name].write(f"Epoch {epoch+1}: Training Loss: {individual_train_losses[dataset_name][-1]}, Validation Loss: {val_loss}, Weighted Validation Loss: {weighted_val_loss}, Mean IoU: {mean_iou}\n")
        ...
        print(f"Site {i+1} - Weighted Validation Loss: {weighted_val_loss}, Mean IoU: {mean_iou}")
        val_losses.append(avg_val_loss)
        print(f"Weighted Global Validation Loss: {weighted_val_loss_sum}")
        ...
        if weighted_val_loss_sum < best_val_loss:
            best_val_loss = weighted_val_loss_sum
            # Save the best model
            torch.save(global_weights, os.path.join(model_directory, f'best_global_model_epoch{epoch}.pth'))
            print(f"Best model saved at epoch {epoch+1} based on weighted validation loss")
            counter = 0
        else:
            counter += 1
            if counter >= args.patience:
                print("Early stopping triggered based on weighted validation loss")
                break
