...
def parse_args():
    parser = argparse.ArgumentParser(description="Pipeline 2: Weighted Federated Averaging - Larger datasets get higher influence in model aggregation")
    ...
    parser.add_argument("--cuda_num", type=int, default=1, help="Cuda device to run on")
...
# Pipeline 2: Weighted federated averaging - larger datasets get proportionally higher influence
def FedAvg_Weighted(models_weights, dataset_sizes):
    """
    Perform weighted federated averaging based on dataset sizes.
    Larger datasets (e.g., Chaksu with 1,345 samples) get higher influence (~24%)
    Smaller datasets (e.g., Magrabi with 94 samples) get lower influence (~2%)
    
    Args:
        models_weights: List of model state dictionaries
        dataset_sizes: List of integers representing the number of samples in each dataset
    
    Returns:
        averaged_weights: Weighted averaged model state dictionary where larger datasets dominate
    """
    num_models = len(models_weights)
    total_samples = sum(dataset_sizes)
    
    # Calculate weights for each model based on dataset size
    weights = [size / total_samples for size in dataset_sizes]
    
    print(f"Dataset sizes: {dataset_sizes}")
    print(f"Averaging weights: {[f'{w:.4f}' for w in weights]}")
    ...
    for key in models_weights[0].keys():
        ...
        weighted_sum = torch.zeros_like(original_tensor, dtype=torch.float32)
        
        # Add weighted contributions from each model
        for i in range(num_models):
            weighted_sum += weights[i] * models_weights[i][key].float()
            
        # Convert back to original dtype and store the weighted average
        averaged_weights[key] = weighted_sum.to(original_tensor.dtype)
...
def main():
    ...
    for epoch in range(args.num_epochs):
        ...
        for i, (model, train_loader, optimizer) in enumerate(zip(models, train_dataloaders, optimizers)):
            ...
            print(f"Dataset {i+1} ({dataset_names[i]}) - Training Loss: {train_loss}")
        ...
        # Pipeline 2: Apply weighted federated averaging (larger datasets dominate)
        global_weights = FedAvg_Weighted(weights, dataset_sizes)
        for model in models:
