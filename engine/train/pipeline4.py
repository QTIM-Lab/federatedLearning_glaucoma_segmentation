...
def parse_args():
    ...
    parser.add_argument("--pretrained_global_model_path", type=str, default="", help="Path to pretrained global model (alias for fl_finetuned)")
    parser.add_argument("--model_dir", type=str, default="", help="Directory to save model checkpoints (overrides default derived path)")
...
def main():
    ...
    # Use explicit model_dir if provided, otherwise derive from output_directory
    if args.model_dir:
    ...
    # Initialize model
    # Support both --fl_finetuned and --pretrained_global_model_path (they're aliases)
    pretrained_path = args.pretrained_global_model_path if args.pretrained_global_model_path else args.fl_finetuned
    ...
    if pretrained_path:
        ...
        print(f"\nLoaded the pretrained/FL-finetuned weights from {pretrained_path}\n")
    else:
        models = [Mask2FormerForUniversalSegmentation.from_pretrained("facebook/mask2former-swin-large-cityscapes-semantic", 
                                                                        id2label=id2label, 
                                                                        ignore_mismatched_sizes=True).to(device) for _ in args.train_csv]
