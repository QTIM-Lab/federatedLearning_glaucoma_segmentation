...
#SBATCH --job-name=origa_globalVal_weightedVal
#SBATCH --gres=gpu:1
#SBATCH --ntasks=16
#SBATCH --output=origa_globalVal_weightedVal.%j.out
#SBATCH --error=origa_globalVal_weightedVal%.err
...
OUTPUT_DIRECTORY="/projects/uthakuria@xsede.org/glaucoma_seg_code/train_outputs/origa_globalVal_weightedVal"
...
MODEL_DIR="/scratch/alpine/uthakuria@xsede.org/fl_glaucoma_checkpoints/origa_globalVal_weightedVal/models"
LAST_CHECKPOINT=$(ls ${MODEL_DIR}/best_global_model_epoch*.pth | sort -V | tail -n 1)

echo "Using checkpoint: ${LAST_CHECKPOINT}"
echo "Starting testing!"

TEST_OUTPUT_DIRECTORY="/projects/uthakuria@xsede.org/glaucoma_seg_code/test_outputs/origa_globalVal_weightedVal"
# test
