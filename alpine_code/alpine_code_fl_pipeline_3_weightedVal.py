...
# required for weighted val
def total_validation_samples(val_dataloaders):
    total_val_samples = 0
    for loader in val_dataloaders:
        total_val_samples += len(loader.dataset)
    return total_val_samples
...
def main():
    ...
    total_val_samples = total_validation_samples(val_dataloaders)
    ...
    for fl_round in range(start_fl_round, fl_rounds):
        ...
        for site_index in range(len(train_dataloaders)):
            ...
            site_val_samples = len(val_loader.dataset)
            site_weight = site_val_samples / total_val_samples
