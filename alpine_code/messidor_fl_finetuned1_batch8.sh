...
#SBATCH --job-name=messidor_fl_finetuned1_batch8
#SBATCH --gres=gpu:1
#SBATCH --ntasks=16
#SBATCH --output=messidor_fl_finetuned1_batch8.%j.out
#SBATCH --error=messidor_fl_finetuned1_batch8%.err
...
OUTPUT_DIRECTORY="/projects/uthakuria@xsede.org/glaucoma_seg_code/train_outputs/messidor_fl_finetuned1_batch8"
...
python3 /projects/uthakuria@xsede.org/glaucoma_seg_code/alpine_fl_train_glaucoma_seg.py \
    ...
    --num_workers 16 \
    --fl_finetuned /scratch/alpine/uthakuria@xsede.org/fl_glaucoma_checkpoints/train_fl_batch8_v2_all9/models/best_global_model_epoch47.pth 

echo "-- train job finished! --"

MODEL_DIR="/scratch/alpine/uthakuria@xsede.org/fl_glaucoma_checkpoints/messidor_fl_finetuned1_batch8/models"
LAST_CHECKPOINT=$(ls ${MODEL_DIR}/best_global_model_epoch*.pth | sort -V | tail -n 1)

echo "Using checkpoint: ${LAST_CHECKPOINT}"
echo "Starting testing!"

TEST_OUTPUT_DIRECTORY="/projects/uthakuria@xsede.org/glaucoma_seg_code/test_outputs/messidor_fl_finetuned1_batch8"
# test
