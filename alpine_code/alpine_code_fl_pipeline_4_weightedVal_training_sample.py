...
# required for weighted val wrt training samples
def total_training_samples(train_dataloaders):
    total_train_samples = 0
    for loader in train_dataloaders:
        total_train_samples += len(loader.dataset)
...
def main():
    ...
    total_train_samples = total_training_samples(train_dataloaders)
    ...
    if latest_checkpoint_path:
        ...
        global_weights = checkpoint  # Assuming checkpoint contains the global model weights
        print(f"Resuming from FL round {start_fl_round + 1} using checkpoint: {latest_checkpoint_path}")
    else:
        print("No checkpoint found. Starting from the beginning.")
        start_fl_round = 0  # Start from the beginning if no checkpoint
    ...
    for fl_round in range(start_fl_round, fl_rounds):
        ...
        round_val_metrics = []
        # Weighted validation loss calculation will be based on validation samples
        round_weighted_val_losses = []
        ...
        for site_index in range(len(train_dataloaders)):
            ...
            site_train_samples = len(train_loader.dataset)
            site_weight = site_train_samples / total_train_samples
            weighted_val_loss = val_loss * site_weight
            round_weighted_val_losses.append(weighted_val_loss)
            
            # And before moving to the next site, cleanup to free up GPU memory
            del model, optimizer, scheduler
            torch.cuda.empty_cache()
            
        # Instead of using the simple mean, use the weighted sum of validation losses
        weighted_global_val_metric = sum(round_weighted_val_losses) / len(round_weighted_val_losses)
        # Check for global early stopping
        stop, global_best_val_metric, early_stop_counter, updated = global_early_stopping_check(weighted_global_val_metric, global_best_val_metric, early_stop_counter, args.fl_patience)
        if stop:
