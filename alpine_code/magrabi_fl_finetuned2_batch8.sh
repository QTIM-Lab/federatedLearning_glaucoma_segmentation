...
#SBATCH --job-name=magrabi_fl_finetuned2_batch8
#SBATCH --gres=gpu:1
#SBATCH --ntasks=16
#SBATCH --output=magrabi_fl_finetuned2_batch8.%j.out
#SBATCH --error=magrabi_fl_finetuned2_batch8%.err
...
OUTPUT_DIRECTORY="/projects/uthakuria@xsede.org/glaucoma_seg_code/train_outputs/magrabi_fl_finetuned2_batch8"

echo "Starting training!"

# train
python3 /projects/uthakuria@xsede.org/glaucoma_seg_code/alpine_fl_train_glaucoma_seg.py \
    --train_csv /projects/uthakuria@xsede.org/glaucoma_seg_code/csvs/magrabi_train.csv \
    --val_csv /projects/uthakuria@xsede.org/glaucoma_seg_code/csvs/magrabi_val.csv \
...
MODEL_DIR="/scratch/alpine/uthakuria@xsede.org/fl_glaucoma_checkpoints/magrabi_fl_finetuned2_batch8/models"
LAST_CHECKPOINT=$(ls ${MODEL_DIR}/best_global_model_epoch*.pth | sort -V | tail -n 1)

echo "Using checkpoint: ${LAST_CHECKPOINT}"
echo "Starting testing!"

TEST_OUTPUT_DIRECTORY="/projects/uthakuria@xsede.org/glaucoma_seg_code/test_outputs/magrabi_fl_finetuned2_batch8"
# test
