...
#SBATCH --job-name=binrushed_fl_finetuned1_batch8
#SBATCH --gres=gpu:1
#SBATCH --ntasks=16
#SBATCH --output=binrushed_fl_finetuned1_batch8.%j.out
#SBATCH --error=binrushed_fl_finetuned1_batch8%.err
...
OUTPUT_DIRECTORY="/projects/uthakuria@xsede.org/glaucoma_seg_code/train_outputs/binrushed_fl_finetuned1_batch8"
...
MODEL_DIR="/scratch/alpine/uthakuria@xsede.org/fl_glaucoma_checkpoints/binrushed_fl_finetuned1_batch8/models"
LAST_CHECKPOINT=$(ls ${MODEL_DIR}/best_global_model_epoch*.pth | sort -V | tail -n 1)

echo "Using checkpoint: ${LAST_CHECKPOINT}"
echo "Starting testing!"

TEST_OUTPUT_DIRECTORY="/projects/uthakuria@xsede.org/glaucoma_seg_code/test_outputs/binrushed_fl_finetuned1_batch8"
# test
