...
#SBATCH --job-name=messidor_globalVal_weightedVal
#SBATCH --gres=gpu:1
#SBATCH --ntasks=16
#SBATCH --output=messidor_globalVal_weightedVal.%j.out
#SBATCH --error=messidor_globalVal_weightedVal%.err
...
OUTPUT_DIRECTORY="/projects/uthakuria@xsede.org/glaucoma_seg_code/train_outputs/messidor_globalVal_weightedVal"
...
python3 /projects/uthakuria@xsede.org/glaucoma_seg_code/alpine_fl_train_glaucoma_seg.py \
    ...
    --num_workers 16 \
    --fl_finetuned /scratch/alpine/uthakuria@xsede.org/fl_glaucoma_checkpoints/globalVal_weightedVal/models/best_global_model_epoch91.pth 

echo "-- train job finished! --"

sleep 5

MODEL_DIR="/scratch/alpine/uthakuria@xsede.org/fl_glaucoma_checkpoints/messidor_globalVal_weightedVal/models"
LAST_CHECKPOINT=$(ls ${MODEL_DIR}/best_global_model_epoch*.pth | sort -V | tail -n 1)

echo "Using checkpoint: ${LAST_CHECKPOINT}"
echo "Starting testing!"

TEST_OUTPUT_DIRECTORY="/projects/uthakuria@xsede.org/glaucoma_seg_code/test_outputs/messidor_globalVal_weightedVal"
# test
