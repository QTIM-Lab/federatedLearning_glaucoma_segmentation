...
#SBATCH --job-name=amd_train_fl_pipeline3_weightedVal
#SBATCH --gres=gpu:1
#SBATCH --ntasks=16
#SBATCH --output=amd_train_fl_pipeline3_weightedVal.%j.out
#SBATCH --error=amd_train_fl_pipeline3_weightedVal%.err
...
OUTPUT_DIRECTORY="/projects/uthakuria@xsede.org/glaucoma_seg_code/train_outputs/amd_train_fl_pipeline3_weightedVal"
...
MODEL_DIR="/scratch/alpine/uthakuria@xsede.org/fl_glaucoma_checkpoints/amd_train_fl_pipeline3_weightedVal/models"
LAST_CHECKPOINT=$(ls ${MODEL_DIR}/best_global_model_round_*.pth | sort -V | tail -n 1)

echo "Using checkpoint: ${LAST_CHECKPOINT}"
echo "Starting testing!"

TEST_OUTPUT_DIRECTORY="/projects/uthakuria@xsede.org/glaucoma_seg_code/test_outputs/amd_train_fl_pipeline3_weightedVal"
# test
