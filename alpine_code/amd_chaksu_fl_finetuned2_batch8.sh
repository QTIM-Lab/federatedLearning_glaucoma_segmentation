...
#SBATCH --job-name=amd_chaksu_fl_finetuned2_batch8
#SBATCH --gres=gpu:1
#SBATCH --ntasks=16
#SBATCH --output=amd_chaksu_fl_finetuned2_batch8.%j.out
#SBATCH --error=amd_chaksu_fl_finetuned2_batch8%.err
...
OUTPUT_DIRECTORY="/projects/uthakuria@xsede.org/glaucoma_seg_code/train_outputs/amd_chaksu_fl_finetuned2_batch8"

echo "Starting training!"

# train
python3 /projects/uthakuria@xsede.org/glaucoma_seg_code/alpine_fl_train_glaucoma_seg.py \
    --train_csv /projects/uthakuria@xsede.org/glaucoma_seg_code/csvs/chaksu_train.csv \
    --val_csv /projects/uthakuria@xsede.org/glaucoma_seg_code/csvs/chaksu_val.csv \
    ...
    --num_workers 16 \
    --fl_finetuned /scratch/alpine/uthakuria@xsede.org/fl_glaucoma_checkpoints/train_fl_new_pipeline_batch8/models/best_global_model_round_9_metric_3.0051.pth 

echo "-- train job finished! --"

MODEL_DIR="/scratch/alpine/uthakuria@xsede.org/fl_glaucoma_checkpoints/amd_chaksu_fl_finetuned2_batch8/models"
LAST_CHECKPOINT=$(ls ${MODEL_DIR}/best_global_model_epoch*.pth | sort -V | tail -n 1)

echo "Using checkpoint: ${LAST_CHECKPOINT}"
echo "Starting testing!"

TEST_OUTPUT_DIRECTORY="/projects/uthakuria@xsede.org/glaucoma_seg_code/test_outputs/amd_chaksu_fl_finetuned2_batch8"
# test
python3 /projects/uthakuria@xsede.org/glaucoma_seg_code/run_inference_multiprocess.py \
    --model_path ${LAST_CHECKPOINT} \
    --input_csv /projects/uthakuria@xsede.org/glaucoma_seg_code/csvs/combined_test.csv \
